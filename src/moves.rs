...
pub struct Move {
    ...
    castling: bool,
    check_opposing_king: bool
}

impl Move {
    pub fn validate(self, mv: Move) -> bool {
        if mv.current_square.row > 8
            || mv.current_square.row < 1
            || mv.current_square.column > 8
            || mv.current_square.column < 1
        {
            return false;
        } 
        else if mv.previous_square.row > 8
            || mv.previous_square.row < 1
            || mv.previous_square.column > 8
            || mv.previous_square.column < 1
        {
            return false;
        } 
        ...
        else if mv.previous_square.piece.color != mv.color
            || mv.current_square.piece.color != mv.color
        {
            return false;
        } 
        else if mv.captured_piece.row != mv.current_square.row
            || mv.captured_piece.column != mv.current_square.column
        {
            return false;
        } 
            ...
            && mv.previous_square.piece != None
            && mv.previous_square.piece != Piece::Pawn
        {
            return false;
        } 
        else if mv.promotion != None
            && mv.color == Color::Black
            && mv.previous_square.piece != None
            && mv.previous_square.piece != Piece::Pawn
        {
            return false;
        ...
        else if mv.castling && mv.previous_square.piece_state.is_some() {
            return false;
        }
        else if mv.castling && mv.current_square.piece_state.piece != Piece::King {
            return false;
        }
...
pub fn create(
    ...
    let mut mv = Move {
        ...
        promotion: promotion,
        castling: castling,
        check_opposing_king: check_opposing_king
    };
...
fn add_pawn_move(
    ...
    if let Some(mv) = create(
        ...
        None,
        false,
        if captured.is_some() && captured.piece == Piece::King { true } else { false }
    ) {
...
pub fn get_knight_moves(state: PieceState, board: &Board) -> Vec<Move> {
    ...
    for &(row, col) in &knight_deltas {
        ...
        if target_row <= 8 && target_row >= 1 && target_col <= 8 && target_col >= 1 {
            let potential_piece: Option<PieceState> =
                board.board[target_row - 1][target_col - 1].piece;
...
pub fn get_bishop_moves(state: PieceState, board: &Board) -> Vec<Move> {
    ...
    for &(dr, dc) in &directions {
        for offset in 1..8 {
            ...
            if let Some(piece_state) = board.board[target_row - 1][target_col - 1].piece {
...
fn is_kingside_castling_valid(king: PieceState, board: &Board, row: u32) -> bool {
    if let Some(rook) = board.board[row][8].piece {
        if rook.piece == Piece::Rook && rook.color == king.color && !rook.has_moved {
            return board.board[row][6].piece.is_none() && board.board[row][7].piece.is_none();
        }
    }
    false
}
